...
void Gearbox_pl_fid_flex::setCurrentRound(int currentRound) {
    ...
    level0ServingB = ((int)(currentRound/4)%2);
    level1ServingB = ((int)(currentRound/(4*4))%2);
    level2ServingB = ((int)(currentRound/(4*4*4))%2);

    level3ServingB = ((int)(currentRound/(4*4*4*4))%2);
    level4ServingB = ((int)(currentRound/(4*4*4*4*4))%2);
...
void Gearbox_pl_fid_flex::enque(Packet* packet) {   
    ...
    */
    ...
    if ((departureRound / (4*4*4*4) - currentRound / (4*4*4*4)) >= 4) {
    ...
    int level0InsertingB = ((int)(departureRound/4)%2);
    int level1InsertingB = ((int)(departureRound/(4*4))%2);

    int level2InsertingB = ((int)(departureRound/(4*4*4))%2);
    int level3InsertingB = ((int)(departureRound/(4*4*4*4))%2);
    ...
    if (departureRound / (4*4*4*4) - currentRound / (4*4*4*4) > 1 || insertLevel == 4) {
        ...
        if (!level3InsertingB) {
            ...
            if (departureRound / (4*4*4) % 4 == 2) {
                ...
                //this->updateFlowPtr(iph->saddr(), iph->daddr(),currFlow);  //12182019 Peixuan
                this->updateFlowPtr(iph->flowid(), currFlow);  // Peixuan 04212020 fid
                thirdLevelB.enque(packet, departureRound / (4*4) % 4);
                ///fprintf(stderr, "Enqueue Level B 3, third FIFO, fifo %d\n", departureRound / (4*4) % 4); // Debug: Peixuan 07072019
            } else {
                currFlow->setInsertLevel(3);
                //this->updateFlowPtr(iph->saddr(), iph->daddr(),currFlow);  //12182019 Peixuan
                this->updateFlowPtr(iph->flowid(), currFlow);  // Peixuan 04212020 fid
                levelsB[3].enque(packet, departureRound / (4*4*4) % 4);
                ///fprintf(stderr, "Enqueue Level B 3, regular FIFO, fifo %d\n", departureRound / (4*4*4) % 4); // Debug: Peixuan 07072019
            }
        }

    
    } else if (departureRound / (4*4) - currentRound / (4*4) > 1 || insertLevel == 2) { //original
        if (!level2InsertingB) {
            /////fprintf(stderr, "Enqueue Level 1\n"); // Debug: Peixuan 07072019
            if (departureRound / (4*4) % 4 == 2) {
                currFlow->setInsertLevel(1);
                //this->updateFlowPtr(iph->saddr(), iph->daddr(),currFlow);  //12182019 Peixuan
                this->updateFlowPtr(iph->flowid(), currFlow);  // Peixuan 04212020 fid
                hundredLevel.enque(packet, departureRound / (4) % 4);
                ///fprintf(stderr, "Enqueue Level 2, hundredLevel FIFO, fifo %d\n", departureRound / (4) % 4); // Debug: Peixuan 07072019
            } else {
                currFlow->setInsertLevel(2);
                //this->updateFlowPtr(iph->saddr(), iph->daddr(),currFlow);  //12182019 Peixuan
                this->updateFlowPtr(iph->flowid(), currFlow);  // Peixuan 04212020 fid
                levels[2].enque(packet, departureRound / (4*4) % 4);
            ...
            if (departureRound / (4*4) % 4 == 2) {
                ...
                //this->updateFlowPtr(iph->saddr(), iph->daddr(),currFlow);  //12182019 Peixuan
                this->updateFlowPtr(iph->flowid(), currFlow);  // Peixuan 04212020 fid
                hundredLevelB.enque(packet, departureRound / (4) % 4);
                ///fprintf(stderr, "Enqueue Level B 2, hundredLevel FIFO, fifo %d\n", departureRound / (4) % 4); // Debug: Peixuan 07072019
            } else {
                currFlow->setInsertLevel(2);
                //this->updateFlowPtr(iph->saddr(), iph->daddr(),currFlow);  //12182019 Peixuan
                this->updateFlowPtr(iph->flowid(), currFlow);  // Peixuan 04212020 fid
                levelsB[2].enque(packet, departureRound / (4*4) % 4);
                ///fprintf(stderr, "Enqueue Level B 2, regular FIFO, fifo %d\n", departureRound / (4*4) % 4); // Debug: Peixuan 07072019
            }
        }
    } else if (departureRound / 4 - currentRound / 4 > 1 || insertLevel == 1) {
        if (!level1InsertingB) {
            /////fprintf(stderr, "Enqueue Level 1\n"); // Debug: Peixuan 07072019
            if (departureRound / 4 % 4 == 2) {
                currFlow->setInsertLevel(0);
                //this->updateFlowPtr(iph->saddr(), iph->daddr(),currFlow);  //12182019 Peixuan
                this->updateFlowPtr(iph->flowid(), currFlow);  // Peixuan 04212020 fid
                decadeLevel.enque(packet, departureRound  % 4);
                ///fprintf(stderr, "Enqueue Level 1, decede FIFO, fifo %d\n", departureRound  % 4); // Debug: Peixuan 07072019
            } else {
                currFlow->setInsertLevel(1);
                //this->updateFlowPtr(iph->saddr(), iph->daddr(),currFlow);  //12182019 Peixuan
                this->updateFlowPtr(iph->flowid(), currFlow);  // Peixuan 04212020 fid
                levels[1].enque(packet, departureRound / 4 % 4);
            ...
            if (departureRound / 4 % 4 == 2) {
                ...
                //this->updateFlowPtr(iph->saddr(), iph->daddr(),currFlow);  //12182019 Peixuan
                this->updateFlowPtr(iph->flowid(), currFlow);  // Peixuan 04212020 fid
                decadeLevelB.enque(packet, departureRound  % 4);
                ///fprintf(stderr, "Enqueue Level B 1, decede FIFO, fifo %d\n", departureRound  % 4); // Debug: Peixuan 07072019
            } else {
                currFlow->setInsertLevel(1);
                //this->updateFlowPtr(iph->saddr(), iph->daddr(),currFlow);  //12182019 Peixuan
                this->updateFlowPtr(iph->flowid(), currFlow);  // Peixuan 04212020 fid
                levelsB[1].enque(packet, departureRound / 4 % 4);
        ...
        if (!level0InsertingB) {
            ...
            //this->updateFlowPtr(iph->saddr(), iph->daddr(),currFlow);  //12182019 Peixuan
            this->updateFlowPtr(iph->flowid(), currFlow);  // Peixuan 04212020 fid
            levels[0].enque(packet, departureRound % 4);
            ...
            //this->updateFlowPtr(iph->saddr(), iph->daddr(),currFlow);  //12182019 Peixuan
            this->updateFlowPtr(iph->flowid(), currFlow);  // Peixuan 04212020 fid
            levelsB[0].enque(packet, departureRound % 4);
...
vector<Packet*> Gearbox_pl_fid_flex::serveUpperLevel(int currentRound) {
    ...
    if (currentRound / (4*4) % 4 == 2) {
        ...
        size = static_cast<int>(ceil((hundredlevelsize + level1size) * 1.0 / (4 - currentRound % 4)));  // 01132020 Peixuan
        ...
        if (!level2ServingB) {
            if (!levels[2].isCurrentFifoEmpty()) {
                int size = static_cast<int>(ceil(levels[2].getCurrentFifoSize() * 1.0 / ((4*4) - currentRound % (4*4))));   // 07212019 Peixuan *** Fix Level 1 serving order (ori)
            ...
            if (!levelsB[2].isCurrentFifoEmpty()) {
                int size = static_cast<int>(ceil(levelsB[2].getCurrentFifoSize() * 1.0 / ((4*4) - currentRound % (4*4))));   // 07212019 Peixuan *** Fix Level 1 serving order (ori)
                //int size = static_cast<int>(ceil((hundredLevel.getCurrentFifoSize() + levels[2].getCurrentFifoSize()) * 1.0 / (10 - currentRound % 10)));  // 07212019 Peixuan *** Fix Level 1 serving order (fixed)
    ...
    if (currentRound / (4*4*4) % 4 == 2) {
        ...
        size = static_cast<int>(ceil((thirdlevelsize + level2size) * 1.0 / ((4*4) - currentRound % (4*4))));  // 01132021 Peixuan *** Fix Level 2 serving order (fixed)
        ...
        if (!level3ServingB) {
            if (!levels[3].isCurrentFifoEmpty()) {
                int size = static_cast<int>(ceil(levels[3].getCurrentFifoSize() * 1.0 / ((4*4*4) - currentRound % (4*4*4))));   // 07212019 Peixuan *** Fix Level 1 serving order (ori)
                //int size = static_cast<int>(ceil((hundredLevel.getCurrentFifoSize() + levels[2].getCurrentFifoSize()) * 1.0 / (10 - currentRound % 10)));  // 07212019 Peixuan *** Fix Level 1 serving order (fixed)
            ...
            if (!levelsB[3].isCurrentFifoEmpty()) {
                int size = static_cast<int>(ceil(levelsB[3].getCurrentFifoSize() * 1.0 / ((4*4*4) - currentRound % (4*4*4))));   // 07212019 Peixuan *** Fix Level 1 serving order (ori)
                //int size = static_cast<int>(ceil((hundredLevel.getCurrentFifoSize() + levels[2].getCurrentFifoSize()) * 1.0 / (10 - currentRound % 10)));  // 07212019 Peixuan *** Fix Level 1 serving order (fixed)
    ...
    if (currentRound / (4*4*4*4) % 4 == 2) {
        ...
        size = static_cast<int>(ceil((forthlevelsize + level3size) * 1.0 / ((4*4*4) - currentRound % (4*4*4))));  // 01132021 Peixuan *** Fix Level 2 serving order (fixed)
        ...
        if (forthLevel.getCurrentFifoSize() && currentRound / 4 % 4 != 5)  // 07222019 Peixuan ***: If hundredLevel not empty, serve it until it is empty (Except Level 1 is serving Convergence FIFO (decade FIFO))
        ...
        int size = static_cast<int>(ceil(levels[4].getCurrentFifoSize() * 1.0 / ((4*4*4*4) - currentRound % (4*4*4*4))));
        for (int i = 0; i < size; i++) {
